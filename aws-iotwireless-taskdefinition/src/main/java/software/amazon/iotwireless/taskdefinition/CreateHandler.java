...
public class CreateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(model, callbackContext)
                .then(progress ->
                        proxy.initiate("AWS-IoTWireless-TaskDefinition::Create", proxyClient, model, callbackContext)
    ...
    private CreateWirelessGatewayTaskDefinitionResponse createResource(
            CreateWirelessGatewayTaskDefinitionRequest createRequest,
            final ProxyClient<IotWirelessClient> proxyClient) {
        CreateWirelessGatewayTaskDefinitionResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(createRequest, proxyClient.client()::createWirelessGatewayTaskDefinition);
